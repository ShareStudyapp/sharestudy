...
const Info = ({ user, isOther, onClickUnFollow, onClickFollow }) => {
  if (!user) {
    return null;
  }
  console.log(user);
  ...
  return (
    <article className="info">
      ...
      {isOther ? (
        <button
          className={user.followerCheck === 1 ? 'following' : ''}
          onClick={user.followerCheck === 1 ? onClickUnFollow : onClickFollow}
        >
          <svg
            width="22"
            height="16"
